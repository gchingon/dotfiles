...
_confsync_message() {
  local status="$1" added modified deleted renamed message
  ...
  added="$(printf '%s\n' "$status" | grep -Ec '^\?\?|^A|^[ MARC]A' || true)"
  modified="$(printf '%s\n' "$status" | grep -Ec '^ M|^M |^MM|^AM|^RM|^CM' || true)"
  deleted="$(printf '%s\n' "$status" | grep -Ec '^ D|^D |^MD|^AD|^RD|^CD' || true)"
  renamed="$(printf '%s\n' "$status" | grep -Ec '^R|^.R' || true)"
...
confsync() {
  ...
  local status ahead behind message
