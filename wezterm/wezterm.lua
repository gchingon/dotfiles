...
-- Copy mode highlighting (copy mode uses the default cursor style)
config.colors = {
...
-- Event to change cursor style in copy mode
...
wezterm.on('update-status', function(window, _)
  ...
  -- Note the use of wezterm.color.parse here, this returns a Color object, which comes with functionality for lightening or darkening the colour (amongst other things).
  local bg = wezterm.color.parse(color_scheme.background)
  local fg = color_scheme.foreground

  -- Each powerline segment is going to be coloured progressively darker/lighter depending on whether we're on a dark/light colour scheme. Let's establish the "from" and "to" bounds of our gradient.
  local gradient_to, gradient_from = bg, bg
  if appearance.is_dark() then
    gradient_from = gradient_to:lighten(0.125)
  else
    gradient_from = gradient_to:darken(0.125)
  end

  -- Yes, WezTerm supports creating gradients, because why not?! Although they'd usually be used for setting high fidelity gradients on your terminal's background, we'll use them here to give us a sample of the powerline segment colors we need.
...
config.keys = {
  -- Sends ESC + b and ESC + f sequence, which is used for telling your shell to jump back/forward.
  {
